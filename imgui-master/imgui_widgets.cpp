...
extern ImFont* IconsFont;
bool ImGui::TabEx(const char* label, const char* icon, const bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (active)
    {
        GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), bb.Max, ImColor(240, 32, 65));
    }
    ...
    if (active || hovered)
    {
        ...
        RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding - ImVec2(11, 13), icon, NULL, &icon_size, style.ButtonTextAlign, &bb);
        ImGui::PopFont();
        RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding + ImVec2(0, 20), label, NULL, &label_size, style.ButtonTextAlign, &bb);
        PopStyleColor();
    }
    else
    {
        PushStyleColor(ImGuiCol_Text, ImVec4(0.78f, 0.78f, 0.78f, 1.f));
        ImGui::PushFont(IconsFont);
        RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding - ImVec2(11, 13), icon, NULL, &icon_size, style.ButtonTextAlign, &bb);
        ImGui::PopFont();
        RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding + ImVec2(0, 20), label, NULL, &label_size, style.ButtonTextAlign, &bb);
...
bool ImGui::Tab(const char* label, const char* icon, const bool active, const ImVec2& size_arg)
{
    return TabEx(label, icon, active, size_arg, 0);
}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2.f) + 3) - ImVec2(0, 3), ImVec2(frame_bb.Max.x - grab_bb.Max.x + grab_bb.Min.x, frame_bb.Max.y - ((frame_bb.Max.y - frame_bb.Min.y) / 2.5f)) + ImVec2(0, 3), ImColor(40, 40, 50), true, 5.f);
    RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2.f) + 3) - ImVec2(0, 3), ImVec2(grab_bb.Min.x, grab_bb.Max.y - ((grab_bb.Max.y - grab_bb.Min.y) / 2.5f)) + ImVec2(0, 3), ImColor(250, 35, 70), true, 5.f);
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x, grab_bb.Min.y + ((grab_bb.Max.y - grab_bb.Min.y) / 2.f) + 2.5), 5.f, ImColor(240, 32, 65), 64);
    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    if (hovered)
